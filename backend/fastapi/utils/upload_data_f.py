...
def _load_service_classes(user_name):
    # Чтение данных из Excel
    df = pd.read_excel(mini.presigned_get_object('user-tabels',f'{user_name.split("/")[0]}/hardcoded/service_codes.xlsx'))
...
def _load_contract_building_relationship(SessionLocall, user_name):
    df = pd.read_excel(mini.presigned_get_object('user-tabels',f'{user_name.split("/")[0]}/hardcoded/contracts_relationship.XLSX'))
...
def _load_fixed_assets(SessionLocall, user_name):
    df = pd.read_excel(mini.presigned_get_object('user-tabels',f'{user_name.split("/")[0]}/hardcoded/main_assets.xlsx'))
...
# def _delete_data(SessionLocall, user_name):
